...
**The dispatched-agent arm: a subagent given a working-directory override.** A Code Audit Team member, or any other agent dispatched into a worktree, is held to a general whole-command test of its own plus two narrower ones, **three** recorded tests, each with its own wording. Three is the recorded count, not a proof that there is no fourth. The general test is **not** the main-thread arm's, despite wording that differs from it only in saying `agent` where that one says `session`: plain capture-into-a-variable runs here (`v="$(pwd)"; echo "$v"`) where the main-thread arm refuses it, which matters because that capture is exactly what the five `code-audit-*` member specs prescribe for the gate handshake, capturing the clearance writer's printed marker path and carrying it to the status push. So the two arms' general tests share a predicate and not a threshold, and reading one arm's admitted shape as the other's stays the error the section opens on.

**Its general whole-command test** refuses a command it cannot verify as a whole even when that command performs no git operation at all: a `python3 <<'PY' … PY` heredoc whose only occurrence of `git` is literal data inside the payload trips it. Naming no git therefore buys a member spec nothing. The wording is the main-thread arm's with `agent` for `session`, and it names git operations regardless of whether the refused command has any:
...
**Why the split is worth recording.** The member specs are written for dispatched members, and dispatched members are what execute them. Driving those same steps by hand from an `EnterWorktree` session hits the main-thread arm, and **that arm's wording** is evidence about the hand-driven case only. Generalizing that wording to the specs reads a prescribed command as unrunnable while the audience it is written for runs it fine, which is a real conclusion to reach and a wrong one. The mirror-image error is just as available and reads the same way: a dispatched member **can** be refused, on any of the three tests above, so meeting a refusal while dispatched is not grounds for dismissing it as an artifact of the hand-driven case. The arms differ; a refusal under one still says nothing about the other, in either direction.
