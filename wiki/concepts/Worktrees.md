...
Everything above is GAIA's model: which tree an action belongs to, and where main is. The Claude Code runtime enforces a **separate** rule of its own on any session working inside a linked worktree, requiring that every operation be verifiably confined to that worktree, and that rule has **two arms**. Which one applies depends on how the session got into the tree: the **main-thread arm** holds a session switched in with `EnterWorktree`, the **dispatched-agent arm** holds a subagent given a working-directory override, a Code Audit Team member or any other agent dispatched into a tree. The arms apply the same tests at different thresholds, so a command a dispatched agent runs without trouble can be refused outright on the main thread, and a refusal seen under one arm says nothing about the other, in either direction.

**The refusal wordings belong to the Claude Code runtime, and this page observes them rather than enumerating them.** Nothing in this repository emits these strings and nothing here can list them, so each test below carries one observed wording as an illustration rather than as a catalogue entry, taken under whichever arm it was observed; the same test under the other arm reads `session` where the quotation says `agent`, or the reverse. Each was observed in September 2026. Successive passes over this section have each met a wording it did not carry, so read what follows as behavior seen, never as a closed set, and expect a refusal whose exact words are not here.

### The tests

**The general whole-command test** refuses a command it cannot verify as a whole even when that command performs no git operation at all: a `python3 <<'PY' … PY` heredoc whose only occurrence of `git` is literal data inside the payload trips it. Naming no git therefore buys a member spec nothing. What it reads is the command as a whole rather than a fixed list of constructs, so what it admits depends on what else the command does: a bare heredoc, pipe, or `&&` chain runs, and that same heredoc composed into a longer command can be refused. It names git operations regardless of whether the refused command has any:

> This agent is isolated in the worktree `<path>`, but this command is too complex to verify that it stays inside the worktree. Refusing to run it - a worktree-isolated agent's git operations must target its own worktree. Split it into plain, separate commands and run them from `<path>`.

**The threshold is where the arms part, and that difference is their whole practical content.** Routing a value through the shell, capturing a command substitution into a variable (`v="$(pwd)"`) or interpolating a shell variable into a path, reliably trips the main-thread arm; the same plain capture (`v="$(pwd)"; echo "$v"`) runs under dispatch. That matters because the capture is exactly what the `code-audit-*` member specs prescribe for the gate handshake, capturing the clearance writer's printed marker path and carrying it to the status push. The arms share this test's predicate and not its threshold, and reading one arm's admitted shape as the other's stays the error this section opens on.

**The repair the message asks for does not cover every command it fires on.** Splitting into plain, separate commands repairs a compound, and for a compound it is the right advice: spell paths out in full and re-read a value rather than capturing it, since shell state does not survive between calls anyway. It repairs nothing when the refused command is already plain and its only mention of the VCS sits in a **text payload** the command carries, a `gh pr create --body-file -` reading a heredoc whose payload is a description containing the word as prose. What trips the test there is the content of an argument handed to another program, so the repair is to move the payload out of argv and into a file and pass the path (`--body-file <path>`), which runs.

**The git-specific whole-command test** refuses a compound that names `git`, in wording that tracks the general test's and names `git` where that one does not:
...
**The per-construct test** refuses a payload whose quoting it cannot verify, a `printf` string carrying unescaped apostrophes for one:

> this command runs a string through `<construct>`, which can't be verified to stay inside the worktree

That wording is an observation under dispatch. Nothing here establishes what the main-thread arm does with the same payload, and its absence from an observation is not evidence that the arm lacks the test.

**Why the split is worth recording.** The member specs are written for dispatched members, and dispatched members are what execute them. Driving those same steps by hand from an `EnterWorktree` session hits the main-thread arm, and **that arm's threshold** is evidence about the hand-driven case only. Generalizing it to the specs reads a prescribed command as unrunnable while the audience it is written for runs it fine, which is a real conclusion to reach and a wrong one. The mirror-image error is just as available and reads the same way: a dispatched member **can** be refused, on any of the tests above, so meeting a refusal while dispatched is not grounds for dismissing it as an artifact of the hand-driven case. The arms share their tests and differ in threshold; a refusal under one still says nothing about the other, in either direction.
