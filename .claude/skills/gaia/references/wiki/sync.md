...
- **Exit 1 carrying either repair section** is the repair case 5c.3 owns, and each attempt counts against its bound:
  - `leaks (N):`, lines shaped `[<check-id>] <path>:<line>  <match>`, a release-scrub check. Only the lines naming a page this run wrote are the repair case; the arm below owns the rest.
...
- **Exit 1 whose `leaks (N):` lines all name files this run did not write** is not the repair case either. The scrub reads the whole staged tree, so a leak can predate the run entirely, and no page this run could edit will clear one. Carry those lines into Step 8's `Leaks not repaired:` field, spend no attempt against 5c.3's bound, and proceed to 5c.4. A re-run still red on nothing but pre-existing lines lands here rather than back in the bound.
