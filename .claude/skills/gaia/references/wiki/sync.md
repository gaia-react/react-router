...
- **Exit 1 carrying `runtime-dependency leaks (N):`** is a shipped script reaching for something the bundle does not carry, and no wiki page can be the cause or the cure. That section comes from a separate verb whose scan set is `.sh` files under the shipped script directories, and `wiki/` is not one of them, so a sync that authored only pages cannot have produced it and no page edit will clear it. Carry the section into the report, spend no attempt against 5c.3's bound, and do not abort the sync on it: it is a pre-existing defect in a shipped script, and it belongs to whoever owns that script.
