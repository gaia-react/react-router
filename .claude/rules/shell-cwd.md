...
One neighbouring mechanism is closed rather than live, and it is worth knowing which: the hooks locate their shared libraries from their own on-disk location, and `.gaia/scripts/lint-hook-cwd-relative-loads.sh` holds the whole hook layer to that, so a `cd` does not reach a library load. It reaches the re-rooting above unchanged, which is enough on its own, because that one fails open across every `PreToolUse` guard at once rather than one gate at a time.
